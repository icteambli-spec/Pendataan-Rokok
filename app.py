...
# B. Baca Google Sheets (Filter Anti-Nyasar & Progress Cerdas)
...
try:
    ...
    if len(all_values) > 1:
        ...
        # === FILTER CERDAS: Abaikan data yang berisi "BELUM INPUT" ===
        if 'TIMESTAMP' in df_gsheets.columns:
            df_gsheets = df_gsheets[
                (~df_gsheets['TIMESTAMP'].astype(str).str.upper().str.contains('BELUM INPUT')) &
                (df_gsheets['TIMESTAMP'].astype(str).str.strip() != '')
            ]
        
        # Hitung Progress Toko Berdasarkan User yang benar-benar Submit
        if 'KODE TOKO' in df_gsheets.columns and not df_gsheets.empty:
    ...
    st.warning(f"⚠️ Sedang merapikan pembacaan Google Sheets... Detail: {e}")
    ...
    if password == "icnbr034":
        ...
        if df_master.empty:
            ...
            if st.button("Download Rekap Lengkap (Excel)", type="primary"):
                # A. Gunakan df_gsheets yang sudah di-filter khusus user submit di atas
